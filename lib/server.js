...
function Server (options) {
  ...
  this._cors = null
  this._directory = '.'
  this._indexUrl = majorNodeVersion > 10
    ? new URL('/index.html')
  ...
  this._handleDirectoryStatsStart = this._onDirectoryStatsStart.bind(this)
...
Server.prototype.start = function start () {
  fs.lstat(this._directory, this._handleDirectoryStatsStart)
}
 ...
 * @private
 */
Server.prototype._onDirectoryStatsStart = function (error, stats) {
  if (error) {
    this._directory = '.'
  } else {
...
Server.prototype._handleStream = function (request, response, requestUrl) {
  ...
  function onOpen () {
    ...
    if (!isReading) {
      ...
      fileInfo = self._parseFilePath(fsPath)
